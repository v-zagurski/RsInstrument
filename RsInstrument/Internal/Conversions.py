...
def str_to_str_or_int(x: str) -> str | int:
	...
	if len(x) == 0:
		return x
	x_trim = Utilities.trim_str_response(x)
	if len(x_trim) < len(x):
		return x_trim
	try:
		int_value = str_to_int(x)
		return int_value
	except ValueError:
		return x_trim
...
def str_to_str_or_int_list(string: str) -> List[str | int]:
	...
	if string.isspace():
		return []
	result = [*map(str_to_str_or_int, string.split(','))]
	return result
...
def str_to_scalar_enum_helper(string: str, scpi_enum: ScpiEnum, array_search: bool, exc_if_not_found) -> Enum:
	...
	value = string.strip() if scpi_enum.has_quotes else Utilities.trim_str_response(string)
