...
class ScpiEnum:
    ...
    def __init__(self, enum_type):
        ...
        self._members_stripped_quotes = None
    ...
    def _init_special_values(self) -> None:
    ...
    def _init_stripped_quotes(self) -> None:
        """Convert the members to the SCPI values without the quotes."""
        if self._members_stripped_quotes is not None:
            return
        self._members_stripped_quotes = []
        for i in range(len(self.members)):
            self._members_stripped_quotes.append(trim_str_response(self.members[i]))
